...
export const Archive = () =>{
    ...
    // הוסף את זה בתחילת הקומפוננטה, אחרי כל ה-useSelector
    console.log("Debug Archive data:", {
        theApplication: theApplication,
        myApplication: myApplication,
        assessmentsDetails: assessmentsDetails,
        apartmentsDetails: apartmentsDetails,
        customersDetails: customersDetails,
        assessorsDetails: assessorsDetails,
        type: type,
        thisAssessor: thisAssessor,
    ...
    const getData = async () => {
      console.log("getData called for customerChose:", customerChose);
    
      if (!customerChose || !customerChose.customerId) {
          console.error("customerChose is missing or invalid:", customerChose);
          return;
      }
    
      const res = await dispatch(yourApplicationsThunk(customerChose.customerId));
      console.log("yourApplicationsThunk result:", res);
    
      if (res.payload != null) {
          console.log("Applications received:", res.payload);
          setApplicationDetails(res.payload);
      } else {
          console.log("No applications received or payload is null");
      }
    }
  
    useEffect(() => {
      console.log("Archive useEffect triggered, type:", type);
    
      // Scroll to top when component mounts
      window.scrollTo(0, 0);

      if (type === "a") {
        console.log("User is assessor, thisAssessor:", thisAssessor);
      ...
      if (!full) {
        console.log("Calling getData for customer applications");
        getData();
      }
    }, []);
    ...
    There is no evaluator assigned to this page yet.
                ...
                >
                  <Typography variant="h6" color="#2c3e50">
                    {!assessmentsDetails || assessmentsDetails.length === 0 ? "Loading assessments..." :
                     !applicationsDetails || applicationsDetails.length === 0 ? "Loading applications..." :
                     "No applications found"}
                  </Typography>
                  <Typography variant="body2" color="#7f8c8d" sx={{ mt: 1 }}>
                    {!assessmentsDetails || !applicationsDetails ? 
